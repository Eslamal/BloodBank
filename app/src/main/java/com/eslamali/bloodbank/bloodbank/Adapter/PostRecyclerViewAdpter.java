...
import com.eslamali.bloodbank.bloodbank.ui.fragment.HomeFragments.HomeFragment.PostsFragment.PostContentFragment;
import com.eslamali.bloodbank.bloodbank.Data.Model.User.registerAndLoginAndEdit.UserData;
import com.eslamali.bloodbank.bloodbank.Data.Model.postToggleFavourite.PostToggleFavourite;
...
public class PostRecyclerViewAdpter extends RecyclerView.Adapter<PostRecyclerViewAdpter.viewHolder> {
    ...
    public PostRecyclerViewAdpter(Context context, List<PostsData> PostsData, Activity activity, boolean favourites, TextView postsFragmentTvNoItems) {
        ...
        user = SharedPrefrance.loadData(activity);
    }
    ...
    private void setData(viewHolder holder, int position) {
        try {
            ...
            HelperMethod.onLoadImageFromUrl(holder.itemPostImgPost, list.get(position).getThumbnailFullPath(), context);
    ...
    private void setAction(viewHolder holder, int position) {
        ...
        holder.view.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                HelperMethod.replaceFragment(homeActivity.getSupportFragmentManager(),
    ...
    private void toggleFavourite(viewHolder holder, int position) {
        ...
        if (list.get(position).getIsFavourite()) {
            holder.itemPostImgLike.setImageResource(R.drawable.afterlike);
            HelperMethod.customToast(activity, activity.getString(R.string.added_to_favourite), false);

        } else {
            holder.itemPostImgLike.setImageResource(R.drawable.beforelike);
            HelperMethod.customToast(activity, activity.getString(R.string.removed_from_favourite), false);
            if (favourites) {
        ...
        RetrofitClient.getClient().getPostToggleFavourite(postsData.getId(), user.getApiToken()).enqueue(new Callback<PostToggleFavourite>() {
            @Override
