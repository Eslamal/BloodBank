...
import com.eslamali.bloodbank.bloodbank.Adapter.SettingNotificationRecyclerAdapter;
import com.eslamali.bloodbank.bloodbank.Data.Model.GenralReqest.GenralReqest;
import com.eslamali.bloodbank.bloodbank.Data.Model.NotificationStting.NotificationStting;
...
public class SettingNotificationFragment extends BaseFragment {
    ...
    private void getSettingNotification() {
        RetrofitClient.getClient().getNotificationSetting(userData.getApiToken()).enqueue(new Callback<NotificationStting>() {
            @Override
    ...
    private void getBlood() {
        RetrofitClient.getClient().getBloodTypes().enqueue(new Callback<GenralReqest>() {
            @Override
    ...
    private void getGover() {
        RetrofitClient.getClient().getGovernorate().enqueue(new Callback<GenralReqest>() {
            @Override
            public void onResponse(Call<GenralReqest> call, Response<GenralReqest> response) {
                try {
                    if (response.body().getStatus() == 1) {
    ...
    private void send() {
        ...
        RetrofitClient.getClient().changeNotificationSetting(userData.getApiToken(), adapterGover.id, adapterBlood.id).enqueue(new Callback<NotificationStting>() {
            @Override
            public void onResponse(Call<NotificationStting> call, Response<NotificationStting> response) {
                try {
                    if (response.body().getStatus() == 1) {
