...
public class LoginFragment extends BaseFragment {
    ...
    private void login() {
        ...
        if (isActive(getActivity())) {
            RetrofitClient.getClient().getUser(Phone, Password).enqueue(new Callback<RegisterAndLoginAndEdit>() {
                @Override
                public void onResponse(Call<RegisterAndLoginAndEdit> call, Response<RegisterAndLoginAndEdit> response) {
                    try {
                        if (response.body().getStatus() == 1) {
                            ...
                            SharedPrefrance.savaData(getActivity(), userData);
                            savaData(getActivity(), SharedPrefrance.REMEMBER, loginCheckBox.isChecked());
                            savaData(getActivity(), SharedPrefrance.USER_PASSWORD, Password);
