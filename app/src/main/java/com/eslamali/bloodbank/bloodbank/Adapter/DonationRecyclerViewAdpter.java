...
import com.eslamali.bloodbank.bloodbank.ui.fragment.HomeFragments.HomeFragment.DonationFragment.DonationDetailsFragment;
...
public class DonationRecyclerViewAdpter extends RecyclerView.Adapter<DonationRecyclerViewAdpter.viewHolder> {
    ...
    public DonationRecyclerViewAdpter(Context context, Activity activity, List<DonationData> donationData) {
        ...
        userData = SharedPrefrance.loadData(activity);
    ...
    public class viewHolder extends RecyclerView.ViewHolder {
        ...
        private void info() {
            if (InternetState.isActive(context)) {
                RetrofitClient.getClient().getDonationDetails(userData.getApiToken(), String.valueOf(donationData.get(position).getId())).enqueue(new Callback<DonationDetailsAndRequest>() {
                    @Override
                    public void onResponse(Call<DonationDetailsAndRequest> call, Response<DonationDetailsAndRequest> response) {
                        try {
                            ...
                            if (response.body().getStatus() == 1) {
                                ...
                                HelperMethod.replaceFragment(home.getSupportFragmentManager(), R.id.home_frame_fragment, donationDetails);
                            }
                        } catch (Exception e) {
